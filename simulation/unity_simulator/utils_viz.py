...
def read_pose_file(file_name, prefix):
    ...
    for l in content:
        if (gpu_flag==True):
            pose.append(cp.array([float(x) for x in l.split(' ')]))
...
def get_skeleton(input_path, prefix, char_id=0):
    ...
    if (gpu_flag==True):
        pose_char = cp.array(skeleton_content)[:, 1:]
        cp.cuda.Stream.null.synchronize()
        frame_index = cp.array(skeleton_content)[:, 1:]
        cp.cuda.Stream.null.synchronize()
    else:
...
def world2im(camera_data, wcoords, imw, imh):
    ...
    if (gpu_flag==True):
        proj = cp.array(camera_data['projection_matrix']).reshape((4,4)).transpose()
        cp.cuda.Stream.null.synchronize()
        w2cam = cp.array(camera_data['world_to_camera_matrix']).reshape((4,4)).transpose()
        cp.cuda.Stream.null.synchronize()
        cw = cp.concatenate([wcoords, np.ones((1, wcoords.shape[1]))], 0) # 4 x N
        cp.cuda.Stream.null.synchronize()
    ...
    if (gpu_flag==True):
        pixelcoords = pixelcoords[:2, :] * cp.array([imw, imh])[:, None]
        cp.cuda.Stream.null.synchronize()
    else:
